...
export async function getServerSideProps({ query: { page = 1} }) {
    const [firstGroup, secondGroup, thirdGroup, fourthGroup, fifthGroup, sixthGroup] = await Promise.all([
        fetch(`https://inventory.dearsystems.com/ExternalApi/v2/Product?page=${page}&IncludeAttachments=true&limit=50`,
        {
            headers: {
        ...
        fetch(`https://inventory.dearsystems.com/ExternalApi/v2/Product?page=${page +1}&IncludeAttachments=true&limit=50`,
        {
            headers: {
              "api-auth-accountid": process.env.REACT_APP_API_ID,
              "api-auth-applicationkey": process.env.REACT_APP_API_KEY,
        ...
        fetch(`https://inventory.dearsystems.com/ExternalApi/v2/Product?page=${page +2}&IncludeAttachments=true&limit=50`,
        {
            headers: {
              "api-auth-accountid": process.env.REACT_APP_API_ID,
              "api-auth-applicationkey": process.env.REACT_APP_API_KEY,
        ...
        fetch(`https://inventory.dearsystems.com/ExternalApi/v2/Product?page=${page +3}&IncludeAttachments=true&limit=50`,
        {
            headers: {
              "api-auth-accountid": process.env.REACT_APP_API_ID,
              "api-auth-applicationkey": process.env.REACT_APP_API_KEY,
   ...
   let data =[];
    ...
    data.push(...first.Products.filter((obj) => obj.Category.includes("Install Supplies")),
              ...second.Products.filter((obj) => obj.Category.includes("Install Supplies")), 
              ...third.Products.filter((obj) => obj.Category.includes("Install Supplies")), 
              ...fourth.Products.filter((obj) => obj.Category.includes("Install Supplies")), 
              ...fifth.Products.filter((obj) => obj.Category.includes("Install Supplies")), 
              ...sixth.Products.filter((obj) => obj.Category.includes("Install Supplies")))
...
const lastPage = Math.ceil(data.length / 15);
  ...
  return (
    <div className="styles.container">
      ...
      <Header/>
        <h1 className={styles.title}>Install Supplies</h1> 
        <SearchButton/>
        ...
        <div className={styles.paginantion}>
          <div className={styles.nextPrevious}>
            <button
              onClick={() => router.push(`/install-supplies/?page=${page - 1}`)}
              disabled={page <= 1}
            >
              Previous
            </button>
